...
export class PixiObject extends Xen.Async {
  ...
  updateObject({x, y, s, r, z}, {object, app}) {
    if (object) {
      let ar = app.stage.width / app.stage.height;
      if (ar > 1) ar = 1 / ar;
      const cx = app.stage.width * ar / 2;
      const cy = app.stage.height * ar / 2;
      const mx = (app.stage.width - cx*2);
      const my = (app.stage.height - cy*2);
      x = (x??0) + mx;
      y = (y??0) + my;
      if (x !== undefined) {
        object.x = x;
