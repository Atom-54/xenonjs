...
export const ImageService = {
  async ResizeImage(atom, {image, size}) {
    if (!image.canvas) {
      image = await ImageService.requireImageCanvas(atom, {image});
    }
  ...
  async requireImageCanvas(atom, {image}) {
    image.canvas = await ImageService.canvasFromImage(atom, image);    
    return image;
  },
  async canvasFromImage(atom, {url}) {
  ...
  async drawImage(atom, {source, target, dx, dy, dw, dh, operation}) {
  ...
  async compose(atom, {images, operations, canvasOut}) {
