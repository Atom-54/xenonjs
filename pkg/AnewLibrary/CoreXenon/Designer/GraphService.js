...
export const GraphService = {
  ...
  GetNodeInfo(layer, atom, {layerId, objectId}) {
    const realLayer = Resources.get(layerId);
    //const designLayer = Design.getDesignLayer(layer);
    //computeAllProperties(designLayer);
    //return getNodeInfo(designLayer, data.objectId);
    return getNodeInfo(realLayer, objectId);
  },
...
const createHostedLayer = async (layer, atom, graph, graphId, designable, composerId) => {
  ...
  if (graphSpec) {
    const [layerId, nodeId, atomName] = Id.splitId(atom.name)
    // const id = `${layerId}::${nodeId}`; //`${makeCapName()}${atom.name}`;
    //const id = makeCapName() + 'Yyz';
    const id = layerId + nodeId;
...
// TODO(sjmiles): name is unclear ... is a very specific 
// tranche of data used (only?) in NodeInspectorAdaptor
const getNodeInfo = async (layer, nodeName) => {
  // result object
  const info = {
    atoms: {}, 
    objectInfo: {}
  ...
  if (layer) {
    ...
    // the layerName + nodeName
    const objectPrefix = Id.joinId(layer.name, nodeName);
    // actual node
    const node = layer.graph.nodes[nodeName];
    // get extended nodeType information
    const nodeType = info.objectInfo.nodeType = getNodeType(node.type);
    // for all atom keys
    for (let qualifiedAtomId of keys(layer.atoms)) {
      // if the atom belongs to this node
