...
import {Flan} from '../Library/CoreFramework/Flan.js';
...
const reifyGraph = async (flan, graph) => {
  ...
  //log(graph);
  //
  const {services, nodeTypes} = await loadLibraries(graph.meta, await Persist.restoreValue('$UserSettings$settings$userSettings'));
  flan.services = services;
  flan.persistations = {$NodeTypeList$typeList$nodeTypes: nodeTypes};
  //
  // create layer
